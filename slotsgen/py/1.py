...
#date = '2025-12-18'
cursor.execute("""
    SELECT bookingid, slt, userid, faculty, course
    FROM slots
    WHERE slt >= %s AND slt < DATE_ADD(%s, INTERVAL 1 DAY)
    ORDER BY slt
""", (date, date))
...
for bookingid, slt, userid, faculty, courseid in rows:
    ...
    btn = '<button class="slot-btn %s" data-bookingid="%s" data-facultyid="%s" data-facultyname="%s" data-courseid="%s" %s>%s</button>' % (
        css_class, bookingid, faculty, faculty_name, courseid, disabled, time_str
    )
